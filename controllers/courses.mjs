...
export default function initCoursesController(db) {
  ...
  const updateInstMigration = async (request, response) => {
    try {
      // THIS ROUTE IS RUN ONCE UPON MIGRATION
      // REASON: upon seeding of data, instructor columns are empty.
      // This route aims to populate these columns
      console.log('bananan');
      ...
      response.send({ courses, allCourseAssArr });
